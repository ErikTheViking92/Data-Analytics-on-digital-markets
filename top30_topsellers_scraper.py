"""Download data about the top N Steam games by Store top sellers.

Workflow:
- Reads Steam API key from `APIkey.txt` (single line with the key). If missing, proceeds without key.
- Scrapes `https://store.steampowered.com/search/?os=win` with filtering by top sellers to get the top games.
- For each top appid, fetches:
  ...
  - SteamDB best-effort data (owners, peak players)

Outputs `top30_topsellers_results.json` and `top30_topsellers_results.csv` by default.
"""
...
def fetch_top_sellers_from_steam_store(n: int = 30) -> List[dict]:
    """Fetch top sellers from Steam Store by scraping the search results page with top sellers sort."""
    url = "https://store.steampowered.com/search/"
    params = {
        "os": "win",
        "sort_by": "trending",  # trending/popular metric includes top sellers
    }
    resp = requests.get(url, params=params, timeout=15, headers={"User-Agent": "steam-scraper/1.0"})
...
def collect_top_sellers(n: int = 30, out_json: str = "top30_topsellers_results.json", out_csv: str = "top30_topsellers_results.csv", use_cache: bool = True):
    ...
    tops = fetch_top_sellers_from_steam_store(n)
    print(f"Found {len(tops)} top sellers from Steam Store")
